...
def rank2(model, inputs, label, k_value, eps, maxiter, boxmax, boxmin, GT, device, lr=1e-2):
    ...
    return purtabed_img_out, modifier_out, attack_success, c_list, purtabed_final_out, predict_label
...
def kfool(model, inputs, GT, k_value, boxmax, boxmin,maxiter, device, lr=1e-2):
    ...
    while (complement_set.issuperset(set(tlab_all))) == False and loop_i < maxiter:
        ...
        F = model(purtabed_img)
        p = torch.argsort(F, dim=1, descending=True)
        purtabed_final_out = F.cpu().detach().numpy()
        _,predict_label_final=nontargeted_TP_index(tlab.cpu().detach().numpy(), F.cpu().detach().numpy(), k_value)
        purtabed_img_out = np.arctanh(((purtabed_img - boxplus) / boxmul * 0.999999).cpu().detach().numpy())
        modifier_out = purtabed_img_out - timg.cpu().detach().numpy()
...
def baselineap(args, model, device, val_loader):
    ...
    if args.app == 'baseline_kfool':
        ...
        if args.eps == 10:
            ...
            # a['random_5'] = [521]  # [247, 874 ,746]
            # a['random_10'] = [521]  # [247, 386, 720]
            a['worst_3'] = [647]  # [614, 647, 1177]
            a['worst_5'] = [647]  # [614, 647, 1177]
            a['worst_10'] = [647]  # [614, 647, 1177]
    elif args.app == 'baseline_kUAP':
        ...
        for ith, (data, GT) in tqdm(enumerate(val_loader)):
            if ith in a['{}_{}'.format(args.label_difficult, args.k_value)]:
                ...
                if index < 1:
                    ...
                    plot_list.append(predict_label)
                    plot_list.append(modifier_out)
                    os.makedirs('./plot_result/{}/{}/{}/eps_{}'.format(args.dataset, args.label_difficult, args.app, args.eps),
                                exist_ok=True)
                    plot_list = np.asarray(plot_list, dtype=object)
                    np.save(
                        './plot_result/{}/{}/{}/eps_{}/images_result_k_{}_ith_{}'.format(args.dataset, args.label_difficult, args.app, args.eps,
                                                                               args.k_value, ith),
                        plot_list)
