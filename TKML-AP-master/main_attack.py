...
def main(args):
    ...
    if args.dataset == 'COCO':
        ...
        DMNIST_samples = utils.get_dmnist_samples(data_dir, device)
    ...
    if args.app == 'train':
        ...
        if args.results == 'dmnist':
            weights_file_path = os.path.join(model_dir, "model-"+args.defense)
        else:
            weights_file_path = os.path.join(model_dir, "model-{}.pth".format(num))
        if os.path.isfile(weights_file_path):
            print("Loading best weights")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='VOC', type=str, choices={'VOC', 'COCO', 'DMNIST'}, help='path to dataset')
