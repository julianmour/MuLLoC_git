...
from CNN_model_x1k import CNN_dmnist as CNNDx1k
from CNN_model_x2 import CNN_dmnist as CNNDx2
from CNN_model_x2 import CNN_tmnist as CNNTx2
from CNN_model_x3 import CNN_dmnist as CNNDx3
from CNN_model_x3 import CNN_tmnist as CNNTx3
from CNN_model_x3 import CNN_voc as CNNPx3
...
def main(args):
    ...
    if model_name == 'cnn':
        if args.dataset == "DMNIST":
            if args.complex == "x2":
                model = CNNDx2()
            elif args.complex == "x3":
                model = CNNDx3()
            elif args.complex == "x1k":
                model = CNNDx1k()
        elif args.dataset == "TMNIST":
            if args.complex == "x2":
                model = nn.DataParallel(CNNTx2())
            elif args.complex == "x3":
                model = nn.DataParallel(CNNTx3())
        elif args.dataset == "PASCAL-VOC":
            if args.complex == "x3":
                model = nn.DataParallel(CNNPx3())

    elif model_name in ['resnet18', 'resnet34', 'resnet50', 'inception_v3']:
    ...
    if args.app == 'train':
        ...
        trn_hist, val_hist = train_model(model, device, optimizer, scheduler, train_loader, valid_loader, model_dir,
                                         num, epochs, log_file)
        ...
        if args.results == 'dmnist' or args.results == "tmnist" or args.results == "pascal-voc":
            weights_file_path = os.path.join(model_dir, args.results + "-" + args.defense + "-" + args.complex)
        else:
            weights_file_path = os.path.join(model_dir, "model-{}.pth".format(num))
        if os.path.isfile(weights_file_path):
            print("Loading best weights")
            model.load_state_dict(torch.load(weights_file_path, map_location=torch.device(device)))
        else:
            raise Exception("Failed to load weights: weights_file_path={} doesnt exist.".format(weights_file_path))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='VOC', type=str, choices={'VOC', 'COCO', 'DMNIST', "TMNIST", 'PASCAL-VOC'},
                        help='path to dataset')
    parser.add_argument('--results', default='woSigmoid-BCE-Adam-bs64-box_-1_1',
    ...
    parser.add_argument('--defense', default='noDef', type=str, help='used defense for model')
    parser.add_argument('--complex', default="x1k", type=str, help='complexity of the model')
    ...
    parser.add_argument('--eps', default=10, type=float, help='eps')
    ...
    parser.add_argument('--label_difficult', default='best', type=str,
                        choices={'best', 'random', 'worst', 'customized', 'one'}, help='difficult types')
    parser.add_argument('--target_labels', default='[]', type=str, help='in case of customized attack')
    parser.add_argument('--attacked_label', default='', type=str, help='in case of one attack')
    parser.add_argument('--app', default='target_attack', type=str, \
                        ...
                        help='attack types')
    parser.add_argument('--norm', default='l2', type=str, choices={'l2', 'lInf'}, help='norm to minimize')
