...
function verify_target_indices(
    ...
    dataset::MIPVerifyMulti.LabelledDataset,
)
    num_samples = MIPVerifyMulti.num_samples(dataset)
...
function initialize_batch_solve(
    save_path::String,
    nn::NeuralNet,
    pp::MIPVerifyMulti.PerturbationFamily,
    norm_order::Real,
    ...
    batch_run_parameters = MIPVerifyMulti.BatchRunParameters(nn, pp, norm_order)
...
function run_on_sample_for_untargeted_attack(
    ...
    solve_rerun_option::MIPVerifyMulti.SolveRerunOption,
)::Bool
    ...
    if solve_rerun_option == MIPVerifyMulti.never
        return !(sample_number in summary_dt[!, :SampleNumber])
    elseif solve_rerun_option == MIPVerifyMulti.always
        return true
    elseif solve_rerun_option == MIPVerifyMulti.resolve_ambiguous_cases
    ...
    elseif solve_rerun_option == MIPVerifyMulti.refine_insecure_cases
        last_solve_status = previous_solves[end, :SolveStatus]
        last_objective_value = previous_solves[end, :ObjectiveValue]
...
+ `solve_rerun_option::MIPVerifyMulti.SolveRerunOption`: Options are
...
function batch_find_untargeted_attack(
    ...
    save_path::String = ".",
    solve_rerun_option::MIPVerifyMulti.SolveRerunOption = MIPVerifyMulti.never,
    pp::MIPVerifyMulti.PerturbationFamily = MIPVerifyMulti.UnrestrictedPerturbationFamily(),
    norm_order::Real = 1,
    tightening_algorithm::MIPVerifyMulti.TighteningAlgorithm = DEFAULT_TIGHTENING_ALGORITHM,
    tightening_options::Dict = MIPVerifyMulti.get_default_tightening_options(optimizer),
    ...
    for sample_number in target_indices
        ...
        if should_run
            ...
            Memento.info(MIPVerifyMulti.LOGGER, "Working on index $(sample_number)")
            input = MIPVerifyMulti.get_image(dataset.images, sample_number)
            true_one_indexed_label = MIPVerifyMulti.get_label(dataset.labels, sample_number) + 1
...
function run_on_sample_for_targeted_attack(
    ...
    elseif solve_rerun_option == MIPVerifyMulti.always
        return true
    elseif solve_rerun_option == MIPVerifyMulti.retarget_infeasible_cases
        last_solve_status = previous_solves[end, :SolveStatus]
