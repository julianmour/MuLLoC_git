...
function Base.show(io::IO, p::Pool)
    ...
    function_display_name = Dict(MIPVerifyMulti.maximum => "max")
    print(
        io,
...
function MaxPool(strides::NTuple{N,Int}) where {N}
    Pool(strides, MIPVerifyMulti.maximum)
end
...
(p::Pool)(x::Array{<:Real}) = MIPVerifyMulti.pool(x, p)
(p::Pool)(x::Array{<:JuMPLinearType}) =
    (Memento.info(MIPVerifyMulti.LOGGER, "Specifying $p ... "); MIPVerifyMulti.pool(x, p))
